...
- **Explicitly pass options to library functions at the call site, instead of relying on the
  defaults**. For example, write `std::stoi(input, nullptr, 10);`
  over `std::stoi(input);`. This improves readability but most of all avoids latent, potentially
...
- Think about performance from the outset, from the beginning. **The best time to solve performance,u  to get the huge 1000x wins, is in the design phase, which is precisely when we can't measure or
...
- Infuse names with meaning. For example, `Allocator& allocator` is a good, if boring name,
  ...
  cleanup should be handled explicitly or by the arena.
...
```cpp
  struct Tracer {
      ...
      static std::expected<Tracer, Error> init(Allocator& gpa, Time time) {
          // ...
      }
  };
```
...
- Use an options struct when arguments can be mixed up. A function taking two `uint64_t` must use an
  options struct. If an argument can be `nullptr`, it should be named so that the meaning of
  `nullptr` literal at the call site is clear.
...
- Pass by value only when copying is intended; otherwise use references and let the type system prevent accidental copies.
...
- Construct larger structs _in-place_ by passing an _out pointer_ during initialization.
  ...
  **Prefer:**
  ```cpp
  struct LargeStruct {
      static std::expected<void, Error> init(LargeStruct* target) {
          *target = LargeStruct{
  ...
  int main() {
      LargeStruct target;  // uninitialized (or default-constructed)
      auto result = LargeStruct::init(&target);
  }
  ...
  **Over:**
  ```cpp
  // Avoided: returning by value
  struct LargeStruct {
      static std::expected<LargeStruct, Error> init() {
          return LargeStruct{
              // potentially moved/copied
          };
      }
  };

  int main() {
      auto result = LargeStruct::init();
      if (result) {
          LargeStruct target = std::move(*result);
      }
  }
...
- Use simpler function signatures and return types to reduce dimensionality at the call site, the
  ...
  `bool` trumps `uint64_t`, `uint64_t64` trumps `std::optional<uint64_t>`, and `std::optional<uint64_t>` trumps `std::expected<uint64_t, E>`.
...
- Use newlines to visually group resource acquisition with its RAII owner’s scope, and separate that group from the rest of the logic in order to
...
- Show your intent with respect to arithmetic. For example, utalize `std::add_sat`, `std::div_sat`, `std::div`
...
- Hard limit all line lengths, without exception, to at most 100 columns for a good typographic
  ...
  structure, close your eyes and let `clang-format` do the rest.
